  ...
  <body>
   <%String userId = request.getParameter("userId");
	...
	<div id="inner">
     ...
     <%} %>      
          分类名称：<a href="javascript:allArticle('<%=userId%>')">所有文章</a>   
    </div>
    ...
    <div id="right" style="width:650px;float:left">
       ...
       <%Set articleList = (Set)request.getAttribute("list"); 
        ...
        %>
         <tr>
           <td><a href="show_Article.jsp?articleId=<%=article.getId()%>"><%=article.getTitle()%></a></td>
                           <td><%=article.getDateline()%></td>
                           <td><%=article.getComment_count()%></td>
          <td><a href="javascript:del('<%=article.getId()%>')">删除</a></td>
          <td><a href="update_Article.jsp?articleId=<%=article.getId()%>&userId=<%=userId%>">修改</a></td>
